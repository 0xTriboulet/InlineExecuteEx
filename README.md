...
Conventional `.cna` scripts register and alias that calls `beacon_inline_execute` to task Beacon to run your BOF. If you want to use `InlineExecuteEx` instead of the default BOF loader for a capability, you can move the `inline-execute-ex.cna` script and the `Release` and `x64` directories to the same directory as your BOF's `.cna` script.
